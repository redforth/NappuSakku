...
1. Unzip `\SF6_Overlay_v1.0.zip` anywhere convenient (Desktop, Downloads, Documents).
2. Double-click `install.bat`. Everything is installed automatically: REFramework, the d2d plugin, the overlay, and frame datafrom FAT.
...
* **Web-app editor** for authoring combos, notes, and video references (runs locally in your browser). Allows for editing combos in the ticker bar in game
...
* **Chracter profile text** under each fighter's name for personal play notes
* **Health-bar tick marks** at 10% intervals so you can see exact damage thresholds and CA
* **Modern Control Scheme Suported** 
...
Copies `NappuSakku\reframework\autorun\Overlay.lua` into `<SF6>\reframework\autorun\`. This is the actual mod file.
...
> \*\*TECH NOTE\*\* What gets written to your SF6 folder:
 ...
 - `<SF6>\reframework\data\sf6_framedata\<CharacterName>\framedata.json` (frame data, 29 folders)
...
1. **Unzip** `SF6_Overlay_v1.0.zip` somewhere convenient. Your Desktop, Downloads, or Documents folder is fine — but **do not delete this folder after install** if you create a desktop shortcut, because the shortcut points back at the `editor\` subfolder inside it.
...
1. In the unzipped `SF6_Overlay_v1.0` folder, open the `editor` subfolder.
...
1. In the unzipped `SF6_Overlay_v1.0` folder, open the `tools` subfolder.
...
1. Download and unzip the new `SF6_Overlay_v1.0.zip` (overwrite the old folder if you want, or extract to a new location).
2. Double-click `install.bat`.
...
1. Delete `<SF6>\reframework\autorun\SF6\Overlay.lua`
2. Delete the entire folder `<SF6>\reframework\data\sf6_framedata\`
3. Delete your unzipped `SF6_Overlay_v1.0` folder
...
* Press **Insert** in-game, open REFramework's **Script Runner**, look for `SF6_Overlay.lua` and check for error messages
* Verify that `<SF6>\reframework\data\sf6\framedata\Ryu\framedata.json` exists (any character folder works for this check)

  ### Editor says "No Python found" or fails to launch

  The bundled Python at `editor\python\` may be missing. Either:

* Run `build\embedded.bat` to rebuild it (requires internet access), or
...
* Check the folder option at the top of the screen is set to 'Street Fighter 6\reframework\data\sf6_framedata'
  ...
  You moved or deleted the unzipped `SF6_Overlay_v1.0` folder after creating the shortcut. To fix:
